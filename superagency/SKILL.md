---
name: superagency
description: Full-stack marketing agency skill: content, campaigns, brand voice, SEO, email sequences, paid ads, CRO, competitive research, PR, social, positioning, personas, analytics, A/B testing, video, podcasts, lifecycle, influencer marketing, events and webinars, community, partnerships, budget planning, creative briefs, crisis comms, performance reporting, pricing and packaging, sales enablement, and marketing operations. Use this whenever the user asks for anything marketing-related — writing copy, planning a launch, auditing SEO, building a nurture flow, checking brand voice, researching competitors, drafting a press release, scripting a video, pitching a podcast, planning spend, pricing a plan, or summarizing metrics — even if they never say the word "marketing" (e.g. "write a LinkedIn post," "why isn't our page ranking," "how do we get on more podcasts," "how do we stop churn," "should we add a cheaper tier," "someone's angry at us on Twitter," "how did we do this week").
---
...
| Pricing tiers, price changes, discounting, price objections | `references/pricing.md` |
| Sales one-pagers, objection docs, demo scripts, pitch decks | `references/sales-enablement.md` |
| Martech stack, lead routing, MQL/SQL, data hygiene | `references/marketing-ops.md` |
...
Pairs that get confused — pick by the cue, not the topic:
- **content vs social** — one specific piece to publish → `content.md`; what to post and how often → `social.md`.
- **cro vs seo** — traffic arrives but doesn't convert → `cro.md`; not enough organic traffic → `seo.md`.
- **email-sequences vs lifecycle** — the emails and their branching → `email-sequences.md`; which stage triggers them and why → `lifecycle.md`.
- **campaigns vs content** — the multi-channel plan and calendar → `campaigns.md`; drafting one asset in it → `content.md`.
- **pr vs crisis** — going after proactive coverage → `pr.md`; responding to something negative → `crisis.md`.
- **positioning vs brand** — what we say and why it's different → `positioning.md`; whether a draft sounds like us → `brand.md`.
- **pricing vs budget** — what to charge customers → `pricing.md`; how to spend the marketing budget → `budget.md`.
...
`draft` `campaign` `voice` `brand` `seo` `sequence` `ads` `cro` `compete` `pr` `social` `position` `persona` `analytics` `test` `video` `lifecycle` `influencer` `event` `community` `partner` `budget` `brief` `crisis` `podcast` `pulse` `trend` `pricing` `sales` `ops`
...
- `sales` = `references/sales-enablement.md`; `ops` = `references/marketing-ops.md`.
...
| `calcheck.py` | Content-calendar dependencies vs due dates | `campaigns.md` |
...
1. **Load saved state.** The brand profile and pulse history don't live in the skill — the user keeps them (see rule 5). At the start of a `brand`, `voice`, `pulse`, or `trend` task, if the user hasn't already pasted a saved block, ask them to paste it if they have one. If not, start from the blank template.
2. **Check `references/brand.md`.** If the profile is still empty and the request is customer-facing, ask 2-3 quick questions (audience, tone adjectives, things to avoid) and save the answers there. Skip for internal or quick asks.
3. **Search the web when the task depends on current facts** — competitor moves, SEO practice, ad specs, platform algorithms, creator rates, disclosure law. Don't answer from memory on anything that changes.
4. **Ask for inputs that materially change the output** (budget, timeline, audience) rather than inventing them. One question, not five.
5. **Persist brand and pulse state as a block, not a repackage.** Edits to `brand.md` and `pulse-log.md` live only in the current conversation. Whenever you write to either, print the **entire** updated file as one fenced code block and tell the user: save this and paste it back at the start of your next session. Do not tell them to rebuild the skill.
